...
export default function CargoSummary({ cargo }) {
  const label = cargoLabel(cargo.cargoCategory, cargo.cargoSubcategory)
  ...
  return (
    <div className="border border-gray-200 dark:border-slate-700 rounded-xl p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-slate-500">Ładunek</h3>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        <div>
          <p className="text-[11px] text-gray-400 dark:text-slate-500">Kategoria</p>
          <p className="text-sm font-medium text-gray-800 dark:text-slate-100 mt-0.5">{label || '-'}</p>
        </div>
        <div>
          <p className="text-[11px] text-gray-400 dark:text-slate-500">Jednostki</p>
          {unit ? (
            <span className="inline-flex items-center gap-1.5 mt-0.5 px-2.5 py-1 rounded-full border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 text-xs font-medium text-emerald-700 dark:text-emerald-300">
              {cargo.packages} × {unit.name} ({unit.code})
            </span>
          ) : (
        ...
        <div>
          <p className="text-[11px] text-gray-400 dark:text-slate-500">Waga</p>
