...
export default function RealShipmentCard({ shipment }) {
  const { t } = useTranslation('pages')
  const { t: tCountry } = useTranslation('countries')
  const branch = BRANCHES[shipment.branch] || BRANCHES.road
  const BranchIcon = branch.icon
  const routeReady = shipment.route.fromCity || shipment.route.toCity
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">
          {routeReady
            ? `${shipment.route.fromCity} (${tCountry(shipment.route.fromCountry, { defaultValue: shipment.route.fromCountry })}) → ${shipment.route.toCity} (${tCountry(shipment.route.toCountry, { defaultValue: shipment.route.toCountry })})`
            : t('tracking.noRoute')}
        </p>
        <p className="text-xs text-gray-500 dark:text-slate-400 truncate mt-0.5">
          {shipment.cargoDescription || t('tracking.noCargoDescription')}
          {shipment.voyage.containerNo && ` · ${shipment.voyage.containerNo}`}
        </p>
      </div>
      ...
      {shipment.dates.eta && (
        <div className="hidden sm:block text-right shrink-0">
          ...
          <p className="text-xs font-medium text-gray-700 dark:text-slate-200">{formatDocumentDate(shipment.dates.eta)}</p>
        </div>
      )}

      <ShipmentStatusBadge status={shipment.status} className="shrink-0" />
