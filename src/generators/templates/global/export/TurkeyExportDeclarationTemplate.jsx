...
export function TurkeyExportDeclarationTemplate({ data }) {
  ...
  return (
    <div style={{ width: '794px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '8px', color: '#000', backgroundColor: '#fff', boxSizing: 'border-box', padding: '8px 10px' }}>
      ...
      <div style={{ border: b, padding: '8px 12px', backgroundColor: '#1a3a6b' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#fff', letterSpacing: '1px' }}>TURKEY — GÜMRÜK BEYANNAMESİ — İHRACAT (EXPORT)</div>
        <div style={{ fontSize: '8px', color: '#a0b8d8', marginTop: '2px' }}>Türkiye Gümrük ve Ticaret Bakanlığı</div>
        <div style={{ fontSize: '6.5px', color: '#a0b8d8', marginTop: '1px' }}>System: BİLGE — Bilgisayarlı Gümrük Etkinlikleri Sistemi</div>
      </div>

      {/* BEYANNAME NO | TARIH | GÜMRÜK MÜDÜRLÜĞÜ */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Beyanname No. / Declaration No.:</div>
          <div style={val} />
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Tarih / Date:</div>
          <div style={val}>{today}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '26px' }}>
          <div style={lbl}>Gümrük Müdürlüğü / Customs Office:</div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '40px' }}>
          <div style={lbl}>İhracatçı / Exporter (adı, adresi, vergi no.):</div>
        ...
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '40px' }}>
          <div style={lbl}>Alıcı / Consignee:</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.receiver?.name}</div>
          <div style={val}>{data.receiver?.address}</div>
        </div>
      </div>

      {/* COUNTRY OF EXPORT | VARIŞ ÜLKESİ | TESLİM ŞEKLİ | FATURA DEĞERİ */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        ...
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Varış Ülkesi / Country of Destination:</div>
          <div style={val}>{data.toCountry || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Teslim Şekli / Incoterms:</div>
          <div style={val}>{data.cargo?.incoterms || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '26px' }}>
          <div style={lbl}>Fatura Değeri / Invoice Value (USD):</div>
          <div style={val}>{data.cargo?.value || ''}</div>
        </div>
      </div>

      {/* SEKCJA: EŞYA BİLGİLERİ / GOODS ITEMS */}
      <div style={{ backgroundColor: '#2c5fa8', border: b, padding: '4px 6px', marginTop: '8px' }}>
        <span style={{ fontSize: '8px', fontWeight: 'bold', color: '#fff' }}>EŞYA BİLGİLERİ / GOODS ITEMS</span>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderTop: b }}>
        ...
        <div style={{ ...thStyle, width: '65px' }}>GTİP Kodu<br />HS Code TR (12)</div>
        <div style={{ ...thStyle, width: '55px' }}>Net Ağırlık<br />kg</div>
        <div style={{ ...thStyle, width: '65px', borderRight: b }}>İstatistiki<br />Değer USD</div>
      </div>
