...
export function UkExportDeclarationTemplate({ data }) {
  ...
  const emptyRow = (
    <div style={{ display: 'flex', minHeight: '18px' }}>
      {[25, null, 55, 40, 40, 55, 55, 55, 65, 65].map((w, i) => (
  ...
  return (
    <div style={{ width: '794px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '8px', color: '#000', backgroundColor: '#fff', boxSizing: 'border-box', padding: '8px 10px' }}>
      ...
      <div style={{ border: b, padding: '8px 12px', backgroundColor: '#1a3a6b' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#fff', letterSpacing: '1px' }}>UK — EXPORT DECLARATION (NES)</div>
        <div style={{ fontSize: '8px', color: '#a0b8d8', marginTop: '2px' }}>HMRC — CDS Customs Declaration Service</div>
        <div style={{ fontSize: '6.5px', color: '#a0b8d8', marginTop: '1px' }}>System: UK CDS (Customs Declaration Service) / CHIEF</div>
      </div>
      ...
      <div style={{ borderLeft: b, borderRight: b, borderTop: b, padding: '4px 6px', backgroundColor: '#fffbe6' }}>
        <span style={{ fontSize: '6.5px', color: '#666' }}>
          <strong>WAŻNE:</strong> Post-Brexit: każdy eksport z UK poza UK wymaga zgłoszenia eksportowego do HMRC przez CDS.
          EORI UK (GB+numer) obowiązkowy.
        </span>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Nr EORI (GB + numer):</div>
      ...
      {/* EKSPORTER | DECLARANT/AGENT */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '36px' }}>
          <div style={lbl}>Eksporter (nazwa, adres, UK):</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.sender?.name}</div>
          <div style={val}>{data.sender?.address}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '36px' }}>
          <div style={lbl}>Declarant / Agent:</div>
          <div style={val} />
        </div>
      </div>

      {/* KOD PROCEDURY | COUNTRY OF DESTINATION | TRANSPORT MODE | PORT OF EXIT */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Kod procedury celnej (np. 1000):</div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderTop: b }}>
        ...
        <div style={{ ...thStyle, width: '55px' }}>Country<br />of Origin</div>
        <div style={{ ...thStyle, width: '65px' }}>Customs<br />Value GBP</div>
        <div style={{ ...thStyle, width: '65px', borderRight: b }}>Commodity<br />Code UK</div>
      </div>
      ...
      <div style={{ borderLeft: b, borderRight: b, borderTop: b, borderBottom: b, padding: '4px 6px' }}>
        <div style={{ ...lbl, fontWeight: 'bold' }}>Supporting Documents (invoice, packing list, licence itp.):</div>
      </div>

      {/* PODPISY */}
      <div style={{ display: 'flex', border: b, marginTop: '8px' }}>
        <div style={{ flex: 1, padding: '5px 7px', borderRight: b, minHeight: '55px', display: 'flex', flexDirection: 'column' }}>
          <div style={lbl}>Exporter / Agent</div>
          <div style={{ flex: 1 }} />
          <div style={{ borderTop: b, paddingTop: '2px', textAlign: 'center', fontSize: '7px', color: '#555' }}>Podpis i pieczęć / Signature &amp; stamp</div>
        </div>
        <div style={{ flex: 1, padding: '5px 7px', borderRight: b, minHeight: '55px', display: 'flex', flexDirection: 'column' }}>
          <div style={lbl}>Licence No. (jeśli)</div>
