...
export function AustraliaImportN10Template({ data }) {
  ...
  return (
    <div style={{ width: '794px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '8px', color: '#000', backgroundColor: '#fff', boxSizing: 'border-box', padding: '8px 10px' }}>
      ...
      <div style={{ border: b, padding: '8px 12px', backgroundColor: '#1a3a6b' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#fff', letterSpacing: '1px' }}>AUSTRALIA — IMPORT DECLARATION N10</div>
        <div style={{ fontSize: '8px', color: '#a0b8d8', marginTop: '2px' }}>Australian Border Force — ABF</div>
        <div style={{ fontSize: '6.5px', color: '#a0b8d8', marginTop: '1px' }}>System: ICS — Integrated Cargo System</div>
      </div>
      ...
      <div style={{ borderLeft: b, borderRight: b, borderTop: b, padding: '4px 6px', backgroundColor: '#fffbe6' }}>
        <span style={{ fontSize: '6.5px', color: '#666' }}>
          <strong>WAŻNE:</strong> Wymagane dla towarów o wartości &gt; 1000 AUD. Przed złożeniem sprawdź wymagania
          biosecurity DAFF — niektóre towary wymagają wcześniejszego zezwolenia.
        </span>
      </div>

      {/* CUSTOMS CLIENT ID | DECLARATION NO */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Customs Client ID:</div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        ...
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '36px' }}>
          <div style={lbl}>Supplier overseas / Dostawca:</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.sender?.name}</div>
          <div style={val}>{data.sender?.address}</div>
        </div>
      </div>

      {/* COUNTRY OF ORIGIN | COUNTRY OF EXPORT | PORT OF ENTRY | CUSTOMS VALUE */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        ...
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '26px' }}>
          <div style={lbl}>Customs Value (AUD):</div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderTop: b }}>
        ...
        <div style={{ ...thStyle, width: '40px' }}>AU Tariff<br />(8)</div>
        <div style={{ ...thStyle, width: '45px' }}>Concession<br />/ FTA</div>
        <div style={{ ...thStyle, width: '40px' }}>Duty<br />Rate %</div>
        <div style={{ ...thStyle, flex: 1, borderRight: b }}>Duty AUD / GST AUD</div>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '22px' }}>
          <div style={lbl}>Biosecurity Declaration (DAFF):</div>
          <div style={val} />
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '22px' }}>
          <div style={lbl}>Treatment required:</div>
          <div style={val} />
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '22px' }}>
          <div style={lbl}>Permit No. (BICON):</div>
      ...
      <div style={{ display: 'flex', border: b, marginTop: '8px' }}>
        <div style={{ flex: 1, padding: '5px 7px', borderRight: b, minHeight: '55px', display: 'flex', flexDirection: 'column' }}>
          <div style={lbl}>Customs Broker (CBFCA)</div>
          <div style={{ flex: 1 }} />
