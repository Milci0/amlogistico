...
export function BrazilDiSiscomexImportTemplate({ data }) {
  ...
  const emptyRow = (
    <div style={{ display: 'flex', minHeight: '18px' }}>
      {[25, null, 45, 35, 35, 40, 45, 45, 40, 40, null].map((w, i) => (
  ...
  return (
    <div style={{ width: '794px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '8px', color: '#000', backgroundColor: '#fff', boxSizing: 'border-box', padding: '8px 10px' }}>
      ...
      <div style={{ border: b, padding: '8px 12px', backgroundColor: '#1a3a6b' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#fff', letterSpacing: '1px' }}>BRAZIL — DECLARAÇÃO DE IMPORTAÇÃO (DI) — SISCOMEX</div>
        <div style={{ fontSize: '8px', color: '#a0b8d8', marginTop: '2px' }}>Secretaria da Receita Federal do Brasil</div>
      ...
      <div style={{ borderLeft: b, borderRight: b, borderTop: b, padding: '4px 6px', backgroundColor: '#fffbe6' }}>
        <span style={{ fontSize: '6.5px', color: '#666' }}>
          <strong>WAŻNE:</strong> Brazylia ma jedną z najdłuższych i najdroższych odpraw celnych na świecie. DI składana
          po przybyciu towaru. Licencja LI (jeśli wymagana) musi być uzyskana PRZED wysyłką.
        </span>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Nr DI (Siscomex):</div>
          <div style={val} />
        </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '36px' }}>
          <div style={lbl}>Importador (nome, endereço):</div>
        ...
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '36px' }}>
          <div style={lbl}>Exportador / kraj:</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.sender?.name}</div>
          <div style={val}>{data.fromCountry || ''}</div>
        </div>
      </div>

      {/* RECINTO ADUANEIRO | MODALIDADE DE PAGAMENTO | VALOR CIF */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Recinto Aduaneiro (porto/aeroporto):</div>
        ...
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '26px' }}>
          <div style={lbl}>Valor CIF (USD):</div>
          <div style={val}>{data.cargo?.value || ''}</div>
        </div>
      </div>

      {/* SEKCJA: ADIÇÕES / GOODS ITEMS */}
      <div style={{ backgroundColor: '#2c5fa8', border: b, padding: '4px 6px', marginTop: '8px' }}>
        <span style={{ fontSize: '8px', fontWeight: 'bold', color: '#fff' }}>ADIÇÕES / GOODS ITEMS</span>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderTop: b }}>
        ...
        <div style={{ ...thStyle, width: '45px' }}>NCM<br />Code (8)</div>
        <div style={{ ...thStyle, width: '40px' }}>ICMS %</div>
        <div style={{ ...thStyle, width: '40px' }}>II<br />(Imposto Import) %</div>
        <div style={{ ...thStyle, flex: 1, borderRight: b }}>IPI % / PIS/COFINS %</div>
      </div>
      ...
      {/* LI / SISCOMEX TAXA / MULTA */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '22px' }}>
          <div style={lbl}>LI No. (Licença de Importação, jeśli dotyczy):</div>
          <div style={val}>{data.customs?.permitNo || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '22px' }}>
          <div style={lbl}>SISCOMEX Taxa:</div>
          <div style={val} />
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '22px' }}>
          <div style={lbl}>Multa (jeśli dotyczy):</div>
          <div style={val} />
        </div>
      </div>
