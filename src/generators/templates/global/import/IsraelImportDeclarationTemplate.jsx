...
export function IsraelImportDeclarationTemplate({ data }) {
  ...
  return (
    <div style={{ width: '794px', fontFamily: 'Arial, Helvetica, sans-serif', fontSize: '8px', color: '#000', backgroundColor: '#fff', boxSizing: 'border-box', padding: '8px 10px' }}>
      ...
      <div style={{ border: b, padding: '8px 12px', backgroundColor: '#1a3a6b' }}>
        <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#fff', letterSpacing: '1px' }}>ISRAEL — IMPORT DECLARATION</div>
        <div style={{ fontSize: '8px', color: '#a0b8d8', marginTop: '2px' }}>Israel Customs Authority — Rashut HaMekhes</div>
        <div style={{ fontSize: '6.5px', color: '#a0b8d8', marginTop: '1px' }}>System: ASYCUDA / Israeli Customs Management System — Misrad HaOtsrot</div>
      </div>
      ...
      <div style={{ borderLeft: b, borderRight: b, borderTop: b, padding: '4px 6px', backgroundColor: '#fffbe6' }}>
        <span style={{ fontSize: '6.5px', color: '#666' }}>
          <strong>WAŻNE:</strong> Niektóre kraje arabskie (Arabia Saudyjska, Algieria, Kuwejt, Irak i inne) wymagają
          'Blacklist Certificate' potwierdzającego brak kontaktów z Izraelem. Sprawdź wymagania kraju docelowego przy
          reeksporcie.
        </span>
      </div>

      {/* H.P. (ח.פ.) | DECLARATION NO. | CUSTOMS HOUSE */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>H.P. importera (ח.פ.):</div>
          <div style={val}>{data.customs?.eori || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Declaration No.:</div>
          <div style={val}>{data.customs?.permitNo || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '26px' }}>
          <div style={lbl}>Customs House (מכס):</div>
          <div style={val}>{data.customs?.exitOffice || ''}</div>
        </div>
      </div>

      {/* IMPORTER | OVERSEAS SUPPLIER */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '36px' }}>
          <div style={lbl}>Importer (name, address):</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.receiver?.name}</div>
          <div style={val}>{data.receiver?.address}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', minHeight: '36px' }}>
          <div style={lbl}>Overseas Supplier:</div>
          <div style={{ ...val, marginTop: '2px' }}>{data.sender?.name}</div>
          <div style={val}>{data.sender?.address}</div>
        </div>
      </div>

      {/* COUNTRY OF ORIGIN | PORT OF ENTRY | CIF VALUE | MODE OF TRANSPORT */}
      <div style={{ display: 'flex', borderLeft: b, borderRight: b, borderTop: b, borderBottom: b }}>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Country of Origin (ארץ מוצא):</div>
          <div style={val}>{data.fromCountry || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>Port of Entry (נמל כניסה):</div>
          <div style={val}>{data.toCity || ''}</div>
        </div>
        <div style={{ flex: 1, padding: '3px 5px', borderRight: b, minHeight: '26px' }}>
          <div style={lbl}>CIF Value (ILS/USD):</div>
      ...
      <div style={{ backgroundColor: '#2c5fa8', border: b, padding: '4px 6px', marginTop: '8px' }}>
        <span style={{ fontSize: '8px', fontWeight: 'bold', color: '#fff' }}>GOODS / תורוחס</span>
      </div>
      ...
      <div style={{ display: 'flex', borderLeft: b, borderTop: b }}>
        ...
        <div style={{ ...thStyle, width: '40px' }}>Israel HS<br />(8)</div>
        <div style={{ ...thStyle, width: '45px' }}>Duty<br />Rate %</div>
        <div style={{ ...thStyle, width: '40px' }}>VAT<br />17%</div>
        <div style={{ ...thStyle, flex: 1, borderRight: b }}>Purchase Tax (jeśli)</div>
      </div>
